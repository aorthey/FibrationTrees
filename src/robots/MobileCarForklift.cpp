...
dart::dynamics::SkeletonPtr MobileCarForklift::MakeSkeleton(const YAML::Node& node) {
  const auto urdf_name = GetDataFolder() + "robots/forklift.urdf";
...
// ompl::multilevel::FactoredSpaceInformationPtr MobileCarForklift::MakeSpaceInformation(const RobotPtr& robot) {
...
// ompl::base::MotionValidatorPtr MobileCarForklift::MakeMotionValidator(const ompl::multilevel::FactoredSpaceInformationPtr& factor, const RobotPtr& /*robot*/) {
//   return std::make_shared<MotionValidatorReedsShepp>(factor);
// }

// std::vector<State3d> MobileCarForklift::GetFK(const StateXd& config) const {
//   auto endeffector = "chassi";
...
// StateXd MobileCarForklift::StateToEigen(const ompl::base::State* state) const {
//   Eigen::VectorXd v(3);
...
// void MobileCarForklift::EigenToState(const StateXd& v, ompl::base::State* state) const {
