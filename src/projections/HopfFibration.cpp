...
HopfFibration::HopfFibration(const ompl::base::SpaceInformationPtr& bundle, const ompl::base::SpaceInformationPtr& base) 
...
void HopfFibration::project(const ompl::base::State *xBundle, ompl::base::State *xBase) const {
...
double HopfFibration::GetLambdaDistance(double lambda, const ompl::base::State *xBundle, ompl::base::State *xFiber) const {
...
void HopfFibration::projectFiber(const ompl::base::State *xBundle, ompl::base::State *xFiber) const {
...
void HopfFibration::lift(const ompl::base::State *xBase, 
  ...
  if(!getFiberSpace()->satisfiesBounds(fiber_state)) {
    OMPL_ERROR("Fiber state is out of bounds");
  ...
  //const auto lambda = 2.0 * pi * rng_.uniformReal(0, 1);
  const auto lambda = (fiber_state->value + pi); //[0, 2*pi]
  const auto theta = s2_state->getTheta(); //[-pi,pi]
  //const auto theta = (s2_state->getTheta() > 0? s2_state->getTheta() : s2_state->getTheta()+pi); //[-pi,pi]
...
ompl::base::StateSpacePtr HopfFibration::computeFiberSpace() {
...
// std::vector<Eigen::Vector3f> HopfFibration::toVector(const ompl::base::State *s1, const ompl::base::State *s2) const {
//   int nd = getBundle()->validSegmentCount(s1, s2);

//   std::vector<Eigen::Vector3f> elements;
//   elements.push_back(toVector(s1));
//   if (nd > 1)
//   {
//     for (int i = 1; i < nd; i++)
//     {
//       getBundle()->interpolate(s1, s2, (double)i / (double)nd, tmpBundle);
//       elements.push_back(toVector(tmpBundle));
//     }
//   }
//   elements.push_back(toVector(s2));
//   return elements;
// }

Eigen::Vector3f HopfFibration::toVector(const ompl::base::State *xBundle) const {
