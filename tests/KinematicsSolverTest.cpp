...
bool CheckStraightLineAccuracy(const KinematicsSolverPtr& kinematics_solver, 
    const std::vector<StateXd>& configs) {
  ...
  for(const auto& config : configs) {
    ...
    if(!maybe_config_frame.has_value()) {
      std::cout << "Could not solve FK for " << config << std::endl;
    }
  ...
  if(max_error > kAccuracyStraightLine) {
    ...
    std::cout << "start << " << configs.front() << std::endl;
    std::cout << "goal  << " << configs.back() << std::endl;
    exit(0);
  }
  return true;
}
...
void CheckFrontAndBackConfigs(const KinematicsSolverPtr& kinematics_solver, const std::vector<StateXd>& configs, const State3d& start_frame, const State3d& goal_frame) {
  ...
  // std::cout << "Start Configuration " << configs.front() << " -> Tcp " << config1_frame << std::endl;
  // std::cout << "Goal Configuration " << configs.back() << " -> Tcp " << config2_frame << std::endl;
  std::cout << "Start Tcp (Desired): " << start_frame << std::endl;
  std::cout << "Start Tcp (Actual) : " << config1_frame << std::endl;
...
void EdgeIKTestFromFrames(const KinematicsSolverPtr& kinematics_solver, 
    const State3d& start_frame, const State3d& goal_frame) {
...
TEST(KinematicsSolverTest, ConfigEdgeIKSameFrameTest) {
  ...
  const auto start_frame = State3d(0.4, 0.4, 0.5);
  ...
  std::cout << "Start (Desired) : " << start << std::endl;
  std::cout << "Goal  (Desired) : " << goal << std::endl;
...
TEST(KinematicsSolverTest, NonFKConfigTest) {
  auto robot = MakeRobot<KukaRobot>();
  KinematicsSolverPtr kinematics_solver = std::make_shared<KinematicsSolver>(robot->GetSkeleton());
  auto start = MakeState({-0.989524, -0.200034, -1.2738, -1.28626, 1.61383, -0.17553, -0.8159});
  auto goal = MakeState({2.78314, 1.41906, -2.96706, -1.4611, -2.07017, 1.40266, 1.89722}); //Does not have FK solution
  auto configs = kinematics_solver->solve_edge_ik_with_config(start, goal);
  EXPECT_EQ(configs.size(), 0u);
...
TEST(KinematicsSolverTest, CannotReachGoalConfigTest) {
  auto robot = MakeRobot<KukaRobot>();
  KinematicsSolverPtr kinematics_solver = std::make_shared<KinematicsSolver>(robot->GetSkeleton());
  auto start = MakeState({-0.503705, -1.08742, -1.14191, -1.98434, 0.363036, -1.08686, -0.0262207});
  auto goal = MakeState({-1.39498, -1.55469, 2.28343, 1.95646, -1.09705, -0.305218, -1.31804});
...
TEST(KinematicsSolverTest, JointFlipTest) {
  auto robot = MakeRobot<KukaRobot>();
  KinematicsSolverPtr kinematics_solver = std::make_shared<KinematicsSolver>(robot->GetSkeleton());
  auto start = MakeState({-1.06341,0.0364156,-1.5652,-1.36632,2.24109,0.174882,-0.205112});
  auto goal  = MakeState({1.28814,-0.0755119,0.874968,-1.04689,1.22978,-0.116029,0.427566});
...
TEST(KinematicsSolverTest, StraightLineTaskSpaceTest) {
  ...
  EdgeIKTestFromFrames(kinematics_solver, State3d(0.7, 0.2, 0.3), State3d(0.5, 0.2, 0.3));
  EdgeIKTestFromFrames(kinematics_solver, State3d(0.6, 0.1, 0.3), State3d(0.6, 0.3, 0.3));
  EdgeIKTestFromFrames(kinematics_solver, State3d(0.6, 0.1, 0.3), State3d(0.4, 0.5, 0.3));
  EdgeIKTestFromFrames(kinematics_solver, State3d(0.6, 0.1, 0.2), State3d(0.4, 0.4, 0.5));
  EdgeIKTestFromFrames(kinematics_solver, State3d(0.4, 0.4, 0.3), State3d(0.4, 0.4, 0.6));
  EdgeIKTestFromFrames(kinematics_solver, State3d(0.3, 0.4, 0.2), State3d(0.6, 0.4, 0.7));
}
